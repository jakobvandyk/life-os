...
export async function buildFocusContext(
  ...
  if ((recentCheckins || []).length > 0) {
    ...
    if (latest.body_fat_pct != null) parts.push(`Body fat: ${latest.body_fat_pct}% (bathroom scale bioimpedance estimate — useful for trends over weeks, not accurate for absolute values)`);
